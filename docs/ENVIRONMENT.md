...
- **Android SDK was not pre-installed**, but this container does have outbound network access (through the pre-configured proxy), so the SDK (`platform-tools`, `platforms;android-34`, `build-tools;34.0.0`) and the Gradle wrapper were installed/generated during Phase 0–1. `./gradlew assembleDebug` and `./gradlew testDebugUnitTest` have been run successfully for `apps/furaxxz` on every phase since — the app **does build** and its JVM unit tests **do pass** (last verified in Phase 7: `app-debug.apk` produced, 34/34 JVM tests green across `CategoryTest`, `JsonParserTest`, `ThemeManifestParserTest`, `PackManifestParserTest`). Maven Central occasionally returns HTTP 429 through the proxy on a cold cache; retrying resolves it. A machine without network access would need a locally provisioned SDK — `furaxxz doctor` reports whether `ANDROID_HOME`/`sdkmanager` are present.
- **`org.json` does not work in local (JVM) Android unit tests** in this project's setup — confirmed empirically in Phase 7 (calling `JSONObject(...)` throws `RuntimeException("... not mocked")` without Robolectric). Rather than add a new test dependency, theme/pack manifest parsing was written as a small dependency-free JSON parser (`engine/json/`) so it's directly unit-testable. Documented here so nobody re-discovers this the hard way.
